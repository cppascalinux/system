...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	if(r<0)
		return r;
	if((r=user_mem_check(e,tf,sizeof(struct Trapframe),PTE_U|PTE_P))<0)
		return r;
	tf->tf_eflags|=FL_IF;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if(ret<0)
		return ret;
	if(va>=UTOP||(uintptr_t)va%PGSIZE)
		return -E_INVAL;
	if((perm&(PTE_P|PTE_U))!=(PTE_P|PTE_U))
		return -E_INVAL;
	int mask=(~0)^PTE_SYSCALL;
	if(perm&mask)
		return -E_INVAL;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// LAB 4: Your code here.
	if(srcva>=UTOP||(uintptr_t)srcva%PGSIZE)
		return -E_INVAL;
	if(dstva>=UTOP||(uintptr_t)dstva%PGSIZE)
		return -E_INVAL;
	if((perm&(PTE_P|PTE_U))!=(PTE_P|PTE_U))
		return -E_INVAL;
	int mask=(~0)^PTE_SYSCALL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if((uintptr_t)srcva<UTOP&&(uintptr_t)e->env_ipc_dstva<UTOP)
	{
		...
		if(!(*pte&PTE_W)&&(perm&PTE_W))
			return -E_INVAL;
		ret=page_insert(de->env_pgdir,pg,de->env_ipc_dstva,perm);
		if(ret<0)
			return ret;
	}
	e->env_ipc_recving=0;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
	case SYS_cputs:
		user_mem_assert(curenv,a1,a2,PTE_U|PTE_P);
	...
	case SYS_env_set_pgfault_upcall:
		return sys_env_set_pgfault_upcall(a1,a2);
